// QVAC Car Nickname Generator — core logic.
// completion() writes the result from a single user input field.
// The personality meter is a deterministic score from keyword density
...
export async function generate(modelId, input) {
  const run = completion({
    ...
    history: [
      {
        role: "system",
        content: ((v) => `Invent one short, fitting nickname (2-4 words) for a car described as: ${v}. Reply with ONLY the nickname, no preamble, no quotation marks.`)(input),
      },
  ...
  return { result, personality: scorePersonality(result) };
}

const KEYWORDS = ["wild", "loyal", "fierce", "steady", "legend", "beast", "rebel"];

function scorePersonality(text) {
  const lower = text.toLowerCase();
  let score = 45;
