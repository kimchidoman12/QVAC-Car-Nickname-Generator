#!/usr/bin/env node
// QVAC Car Nickname Generator — GUI mode.
...
const PORT = process.env.PORT ? Number(process.env.PORT) : 29361;
...
async function main() {
  ...
  const server = http.createServer(async (req, res) => {
    ...
    if (req.method === "POST" && req.url === "/api/carnickname") {
      try {
        const { car } = await readBody(req);
        if (!car || !car.trim()) {
          res.writeHead(400, { "Content-Type": "application/json" });
          res.end(JSON.stringify({ error: "Please fill in the field first" }));
          return;
        }
        const result = await generate(modelId, car.trim());
  ...
  server.listen(PORT, () => {
    console.log(`▸ QVAC Car Nickname Generator GUI ready at http://localhost:${PORT}`);
  });
